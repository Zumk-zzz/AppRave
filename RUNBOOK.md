...
Семь наборов подряд: сервер сам по себе (вход, покупка, гонка за последний
...
со складом, возврат денег за отменённую вечеринку и бизнес-правила
(срок отмены, запрет покупки задним числом, возврат баллов). Все зелёные —
система в порядке.
...
`npm run smoke:refund`, `npm run smoke:rules`.

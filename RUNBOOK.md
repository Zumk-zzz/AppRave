...
Четыре набора подряд: сервер сам по себе (вход, покупка, гонка за последний
билет, двойная бронь стола, оплата, отмена), права ролей, вход по телефону
и почте, и заказы теми же вызовами, что делает приложение. Все зелёные —
система в порядке.

Отдельно любой из наборов: `npm run smoke`, `npm run smoke:roles`,
`npm run smoke:identity`, `npm run smoke:orders`.

...
**Проверочные вечеринки засорили афишу.** Наборы smoke заводят свои события,
и за десяток прогонов их набирается столько, что рабочая афиша тонет среди
«ПРОШЛА mufpl» и «SMOKE NIGHT». Через приложение их не вычистить — по части
есть оплаченные заказы:
...
Удаляет только свои: узнаёт по названиям, которыми их называют проверки.
Заодно разжалует проверочных сотрудников — наборы заводят их десятками,
и в списке команды они выглядят как настоящие бармены.

После уборки стоит выполнить `npm run seed`: он вернёт демо-роли
900 000-00-01…03, если проверки успели их перезаписать.

**Баллы разошлись с историей.** Снятие баллов при отмене появилось позже
начисления, поэтому у тех, кто отменял заказы раньше, на счету осталось
лишнее. Счётчик накопительный, сам он не выправится:
...
Приводит баллы и уровень к сумме начислений по состоявшимся заказам.
